...
If you're developing a 3D graphical app and aiming for a 10% improvement and a 10x difficulty scale compared to the standard rasterization approach, you should try implementing real-time ray tracing. God Bless you're fortunate enough to have access to a decent IDE that supports shader debugging, such as XCode and the Metal API. However, if you don't have such a setup, be ready to spend countless hours in troubleshooting CPU <=> GPU synchronization problems, padding rules, and silent errors. Note: even with a good setup you're restricted to a specific platform and can't share the result with someone who doesn't have the required hardware and software.
...
Real-time ray tracing doesn't worth the effort. You should always prioritize the raster approach. However, there's hope: you can generate ray tracing **offline** and create a video. Offline rendering doesn't have performance problems because a slide or video is generated without the need for two separate programming languages and some complex GPU/CPU synchronization wizardy.

Additionally, reality isn't always fun. People use apps to escape it. Can you justify the need to emulate the reality, or is it because you want to showcase your skills and create something more realistic for the sake of realism?